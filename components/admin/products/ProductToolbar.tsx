...
export function ProductToolbar({
  ...
  const activeFilters = [
    ...(query
      ? [{ key: "query" as const, label: t("Search: {query}", { query }) }]
      : []),
    ...(stock
      ? [
          {
            key: "stock" as const,
            label: stock === "low" ? t("Low stock") : t("Out of stock"),
          },
        ]
      : []),
    ...(includeInactive
      ? [{ key: "includeInactive" as const, label: t("Including archived") }]
      : []),
  ];
    ...
    <div className="mb-4 grid min-w-0 shrink-0 gap-4 max-[641px]:gap-3">
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2.5 gap-y-1.5">
          ...
          >
            {total === undefined
              ? t("Loading…")
              : t(total === 1 ? "{count} product" : "{count} products", {
      ...
      >
        ...
        >
          ...
          >
            <span className="max-[401px]:sr-only">{t("Search")}</span>
            <AdminIcon
              name="search"
              size={16}
      ...
      {activeFilters.length > 0 && (
        <div
          ...
          role="group"
          aria-label={t("Active product filters")}
        >
